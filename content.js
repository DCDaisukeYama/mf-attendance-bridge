...
const DEFAULTS = {
  ...
  outKeywords: ["退勤","退社","打刻終了","勤務終了","check out","clock out"],
...
// デバッグ用ログ出力関数
function log(...args){ if (cfg.debug) console.log("[MF-Bridge]", ...args); }
...
async function loadCfg(){
  ...
  if (hasChromeStorage()) {
    try {
      ...
      log("loaded cfg:", cfg);
      return;
    } catch(e) { console.warn("[MF-Bridge] storage read failed:", e); }
  }
  cfg = { ...DEFAULTS };
...
function onAnyClickLike(e, phase){
  ...
  if (!hit) {
    log(`${phase} miss`, {
      target: e.target && e.target.outerHTML ? e.target.outerHTML.slice(0,120) + "..." : String(e.target),
  ...
  log(`${phase} hit ->`, { action: hit.action, reason: hit.reason, node: hit.node.tagName, text: normalizeText(hit.node) });
...
(async function init(){
  await loadCfg();
  window.addEventListener("pointerdown", (e)=>onAnyClickLike(e, "pointerdown"), true);
